...
from src.calibration.calibrators import MultiCalibrator, ProbabilityCalibrator
...
class TestConformalPrediction:
    ...
    def test_conformal_fit(self, trained_model, synthetic_data):
        ...
        cp = ConformalPredictor(base_estimator=trained_model, alpha=0.1, cv=3)
...
class TestCalibrationPipeline:
    ...
    def test_pipeline_predict(self, trained_model, synthetic_data):
        ...
        pipeline = CalibrationPipeline(
            base_estimator=trained_model, use_conformal=True, use_calibration=True
        )
    ...
    def test_pipeline_without_conformal(self, trained_model, synthetic_data):
        ...
        pipeline = CalibrationPipeline(
            base_estimator=trained_model, use_conformal=False, use_calibration=True
        )
    ...
    def test_pipeline_without_calibration(self, trained_model, synthetic_data):
        ...
        pipeline = CalibrationPipeline(
            base_estimator=trained_model, use_conformal=True, use_calibration=False
        )
        ...
        assert np.allclose(
            results["probabilities"], results["uncalibrated_probabilities"]
        )
