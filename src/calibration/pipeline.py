...
from .calibrators import MultiCalibrator, ProbabilityCalibrator
...
class CalibrationPipeline:
    ...
    def predict(self, X: np.ndarray, return_uncertainty: bool = False) -> dict:
        """
        ...
        if (
            return_uncertainty
            and self.use_conformal
            and self.conformal_predictor is not None
        ):
    ...
    def evaluate_calibration(
        ...
        metrics = {
            ...
            "improvement": uncalib_metrics["ece"] - calib_metrics["ece"]
            if uncalib_metrics["ece"]
            else None,
            "n_bins": n_bins,
            "n_samples": len(y),
        ...
        if plot:
            self._plot_reliability_diagram(
                uncalibrated_probs, calibrated_probs, y, n_bins, save_path
            )
    ...
    def _plot_reliability_diagram(
        ...
        ax1.scatter(
            bin_probs_uncal, bin_freqs_uncal, s=bin_counts_uncal, alpha=0.6, c="red"
        )
        ...
        ax2.scatter(
            bin_probs_cal, bin_freqs_cal, s=bin_counts_cal, alpha=0.6, c="green"
        )
    ...
    def predict_race(self, X_race: np.ndarray, normalize: bool = True) -> dict:
        """
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(
        f"Dataset: {len(X_train)} train, {len(X_cal)} calibration, {len(X_test)} test"
    )
    ...
    if "uncertainty" in race_predictions:
        ...
        for i, size in enumerate(set_sizes):
            ...
            print(
                f"  Horse {i + 1}: Prediction set size={size} ({confidence} confidence)"
            )
