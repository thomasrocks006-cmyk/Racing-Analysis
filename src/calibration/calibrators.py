...
class ProbabilityCalibrator:
    ...
    def evaluate(self, probabilities: np.ndarray, labels: np.ndarray, n_bins: int = 10) -> dict:
        probabilities = np.clip(probabilities, 1e-7, 1 - 1e-7)
...
class MultiCalibrator:
    ...
    def __init__(self, methods: list | None = None):
        ...
        self.calibrators = {method: ProbabilityCalibrator(method) for method in self.methods}
        self.best_method = None
        self.best_calibrator = None
        self.validation_scores = {}
        
    def fit(self, probabilities: np.ndarray, labels: np.ndarray, validation_split: float = 0.2):
        probs_train, probs_val, labels_train, labels_val = train_test_split(
            probabilities, labels, test_size=validation_split, random_state=42, stratify=labels
        )
        ...
        logger.info(f"Best method: {self.best_method} (ECE={self.validation_scores[self.best_method]:.4f})")
...
if __name__ == "__main__":
    ...
    for method in ['isotonic', 'platt']:
        ...
        print(f"  Before: {ece_before:.4f} | After: {ece_after:.4f} | Improvement: {ece_before-ece_after:.4f}\n")
