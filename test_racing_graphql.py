...
def test_scrape_race():
    ...
    try:
        ...
        print(
            f"   Track: {race_card.race.track_condition} ({race_card.race.track_type.value if race_card.race.track_type else 'N/A'})"
        )
        ...
        for i, run in enumerate(race_card.runs[:10], 1):  # Show first 10
            horse = next(
                (h for h in race_card.horses if h.name == run.horse_name), None
            )
            gear = next(
                (g for g in race_card.gear if g.horse_name == run.horse_name), None
            )

            print(f"\n   {i}. {run.horse_name}")
            if horse:
                print(
                    f"      Age/Sex: {horse.age}yo {horse.sex.value} | Colour: {horse.colour}"
                )
