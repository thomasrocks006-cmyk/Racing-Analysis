...
from src.data.models import Horse, Jockey, Race, Result, Run, Trainer
...
class HistoricalDataBackfill:
    ...
    def backfill_all_venues(self, start_date: date, end_date: date):
        ...
        logger.info(
            f"Target: {total_races_target} races across {len(self.VENUES)} venues"
        )

        for venue, venue_config in self.VENUES.items():
            logger.info(f"\n{'=' * 70}")
            logger.info(
                f"VENUE: {venue.upper()} (target: {venue_config['min_races']} races)"
            )
            logger.info(f"{'=' * 70}")
    ...
    def backfill_venue(self, venue: str, start_date: date, end_date: date):
        ...
        while current_date <= end_date:
            ...
            try:
                ...
                self.stats["errors"].append(
                    {"venue": venue, "date": current_date, "error": str(e)}
                )
                current_date += timedelta(days=1)
    ...
    def backfill_race(
        self, venue: str, race_date: date, race_num: int, race_data: dict
    ):
    ...
    def _process_runner(self, race_id: str, runner_data: dict):
        ...
        # Extract horse
        horse_id = runner_data.get(
            "horse_id", f"HORSE-{runner_data.get('name', 'UNKNOWN')}"
        )
    ...
    def print_summary(self):
        ...
        success_rate = (
            ...
            if self.stats["races_attempted"] > 0
            else 0
        )
...
def main():
    """Main entry point"""
    parser = argparse.ArgumentParser(description="Backfill historical racing data")
    parser.add_argument("--venue", help="Single venue to backfill")
    parser.add_argument(
        "--start",
    ...
    parser.add_argument("--all", action="store_true", help="Backfill all venues")
    parser.add_argument(
        "--sample", action="store_true", help="Backfill 20 races for testing"
    )
